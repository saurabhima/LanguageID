...
csv_file_absolute_path=os.path.join(config.DATASET_CSV_DIR,config.DATASET_CSV_FILE)
print csv_file_absolute_path
file = open(csv_file_absolute_path, 'r')
for iter, line in enumerate(file.readlines()[1:]): # first line of traininData.csv is header (only for trainingData.csv)
    ...
    for augmentIdx in range(0, 20):
        ...
        plot_abs_path=os.path.join(config.PLOTS_DIR,filename+'.'+str(augmentIdx)+'.png')
        plotstft(wavfile, channel=0, name=plot_abs_path,alpha=alpha, offset=offset)
